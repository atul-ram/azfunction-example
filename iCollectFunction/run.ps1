...
## Log Entry format 2.0 from URL https://docs.microsoft.com/en-us/rest/api/storageservices/storage-analytics-log-format#log-entry-format-20
...
$containers | ForEach-Object {
    ...
    do {
        $blobs = Get-AzStorageBlob -Context $storageContext -Container $container.Name -MaxCount $maxReturn -ContinuationToken $token | Where-Object LastModified -ge (Get-Date).AddMinutes(-5)
        if($Null -eq $blobs) {
            break
        }
        ...
        foreach($blob in $blobs)
        {
            ...
            $lines = Get-Content $filename     # fetch file from temp location
